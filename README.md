...
- Generate Lexer files with `npm run antlr`
- Generate parser files with `npm run antlr2`
- Build program with `npm run react-build`
- Start the program with `npm run react-run`
...
The API library provides a number of pre-defined functions that quiz developers can use for creating their quizes.
