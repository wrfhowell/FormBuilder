...
export class ParseTreeToAST
  ...
  implements FormGeneratorParserVisitor<Node>
{
  visitProgram(ctx: ProgramContext): Program {
    if (ctx.variables()?.variables_object()) {
      return new Program(
        <Pages>ctx.pages().accept(<FormGeneratorParserVisitor<Node>>this),
        <VariablesArray>ctx
          .variables()
          ?.variables_object()
          .accept(<FormGeneratorParserVisitor<Node>>this)
      );
    } else {
      return new Program(
        <Pages>ctx.pages().accept(<FormGeneratorParserVisitor<Node>>this),
        ...
        variableList.push(
          new Variable(
            ...
            <(string | number | undefined)[]>(
              varValue.array()?.accept(<FormGeneratorParserVisitor<Node>>this)
            )
          )
        );
      } else {
  ...
  visitPage(ctx: PageContext): Page {
    ...
    let goTo = <GoTo_Object>pageFields
      .goTo_field()
      ?.goTo_object()
      .accept(<FormGeneratorParserVisitor<Node>>this);
    let header = <string | Expression | VariableName | undefined>(
      pageFields.header_field()?.accept(<FormGeneratorParserVisitor<Node>>this)
    );
    let instructions = <string | Expression | VariableName | undefined>(
      pageFields
        .instructions_field()
        ?.accept(<FormGeneratorParserVisitor<Node>>this)
    );
    let displayQuestions: boolean | undefined;
    let questionArray: Question_Array = <Question_Array>pageFields
      .questions_field()
      ?.question_array()
      .accept(<FormGeneratorParserVisitor<Node>>this);
    ...
    if (pageFields.displayQuestions_field()?.boolean()) {
      displayQuestions =
        Number(pageFields.displayQuestions_field()?.boolean().text) == 1;
    } else {
      displayQuestions = undefined;
    }
    if (pageFields.variables()?.variables_object()) {
      varsObject = <VariablesArray>pageFields
        .variables()
        ?.variables_object()
        .accept(<FormGeneratorParserVisitor<Node>>this);
    } else {
  ...
  ): string | Expression | VariableName | undefined {
    const headerString = ctx.STRING()?.text;
    const headerExpression = <Expression>(
      ctx.expression()?.accept(<FormGeneratorParserVisitor<Node>>this)
    );
  ...
  visitQuestion_array(ctx: Question_arrayContext): Question_Array {
    ...
    for (const q: QuestionContext in ctx.question()) {
      questionList.push(
        <Question>q.accept(<FormGeneratorParserVisitor<Node>>this)
      );
    }
  ...
  visitQuestion(ctx: QuestionContext): Question {
    ...
    let label = <string | Expression | VariableName | undefined>(
      questionFields
        .label_field()
        ?.accept(<FormGeneratorParserVisitor<Node>>this)
    );
    ...
    let displayIf = <string | Regex | Expression | undefined>(
      questionFields
        .displayIf_field()
        ?.accept(<FormGeneratorParserVisitor<Node>>this)
    );
    ...
    let correctAnswer = <string | number | Regex | Expression | undefined>(
      questionFields
        .correctAnswer_field()
        ?.accept(<FormGeneratorParserVisitor<Node>>this)
    );
    ...
    if (questionFields.isRequired_field()?.boolean()) {
      isRequired =
        Number(questionFields.isRequired_field()?.boolean().text) == 1;
    } else {
  ...
  visitOptions_field(ctx: Options_fieldContext): Options {
    return new Options(
      <Option[]>ctx.array().accept(<FormGeneratorParserVisitor<Node>>this)
    );
  }
  ...
  ): string | Regex | Expression | undefined {
  ...
  ): string | number | Regex | Expression | undefined {
    ...
    if (stringExpression) {
      return new Expression(
        new StringExpression(
          <string>(
            stringExpression.accept(<FormGeneratorParserVisitor<Node>>this)
          )
        )
      );
      ...
      return new Expression(
        new MathExpression(
          <string>mathExpression.accept(<FormGeneratorParserVisitor<Node>>this)
        )
      );
    }
  }
  ...
  visitString_expression(ctx: String_expressionContext): string {
    ...
    if (finalExpression) {
      ...
      if (finalExpression.string_expression_val2()) {
        return (
          strValue + finalExpression.string_expression_val2()?.STRING().text
        );
      ...
      return (
        extendedExpression.STRING()?.text +
  ...
  visitGoTo_object(ctx: GoTo_objectContext): GoTo_Object {
    return new GoTo_Object(
      ctx.STRING()?.text,
      <{ [key: string]: string }>(
        ctx.go_object()?.accept(<FormGeneratorParserVisitor<Node>>this)
      )
    );
  }
  ...
  visitGo_object(ctx: Go_objectContext): { [key: string]: string } {
    ...
    for (let i: number = 0; i < ctx.go_object_key().length; i++) {
      goToDictionary[ctx.go_object_key()[i].STRING().text] = ctx
        .go_object_value()
        [i].STRING().text;
    }
