import { Program, Pages, Page, Node, Question } from "../export";
import {
  ArrayContext, Boolean_expressionContext, Cond_bodyContext, ConditionalContext,
  CorrectAnswer_fieldContext,
  DisplayIf_fieldContext, DisplayQuestions_fieldContext,
  ExpressionContext,
  Form_state_accessContext,
  Function_arrayContext,
  Function_bodyContext, Function_callContext,
  Function_paramsContext,
  FunctionContext, GoTo_fieldContext,
  Header_fieldContext,
  Instructions_fieldContext,
  Label_fieldContext,
  Math_expressionContext, Num_boolean_expressionContext,
  Options_fieldContext, Page_fieldContext,
  PageContext,
  PagesContext,
  ParameterContext,
  Question_arrayContext,
  QuestionContext, Scoped_expressionContext, StatementContext,
  Static_functionContext, String_boolean_expressionContext,
  String_expressionContext, Unscoped_expressionContext,
  VariableContext,
  ...
  implements FormGeneratorParserVisitor<any>
{
  ...
  visitProgram(ctx: ProgramContext): Program {
    ...
    const functionsObject = <Functions_Array> ctx.functions()?.function_array().accept(this);
    const variablesObject = <VariablesArray> ctx.variables()?.variables_object().accept(this);
  ...
  visitPage(ctx: PageContext): Page {
    ...
    for (let pageFields of ctx.page_fields().page_field()) {
      if (pageFields.id_field()) {
        let id = pageFields.id_field()?.STRING()?.text || pageFields.id_field()?.expression()?.accept(this);
      } else if (pageFields.header_field()) {
        let header = <string | Expression | VariableName | Function_Call | undefined>(
            pageFields.header_field()?.accept(this)
        );
      } else if (pageFields.instructions_field()) {
        let instructions = <string | Expression | VariableName | Function_Call | undefined>(
            pageFields
                .instructions_field()
                ?.accept(this)
        );
        ...
        let goTo = pageFields
            .goTo_field()?.accept(this);
      } else if (pageFields.displayQuestions_field()) {
        let displayQuestions = pageFields.displayQuestions_field()?.accept(this);
      } else if (pageFields.questions_field()) {
        questions = <Question_Array>pageFields
          .questions_field()
          ?.question_array()
          .accept(this);
      }
  ...
  visitFunction_params(ctx: Function_paramsContext): (string | number | Expression | VariableName | StaticFunction |
      Function_Call | FormStateAccess | undefined)[] {
    let parameters: (string | number | Expression | VariableName | StaticFunction |
        Function_Call | FormStateAccess | undefined)[] = [];
  ...
  visitParameter(ctx: ParameterContext): string | number | Expression | VariableName | StaticFunction |
      Function_Call | FormStateAccess | undefined {
    ...
    const formStateAccess = <FormStateAccess>(ctx.form_state_access()?.accept(this));
    if (funcString) {
      return funcString;
    } else if (funcNum) {
  ...
  visitStatic_function(ctx: Static_functionContext): StaticFunction {
    return new StaticFunction(ctx.STATIC_FORM_NAME().text, ctx.function_params().accept(this));
  }

  visitForm_state_access(ctx: Form_state_accessContext): FormStateAccess {
    return new FormStateAccess(ctx.path_to_key().path_to_page_key().STRING().text,
        ctx.path_to_key().path_to_question_key().STRING().text);
  }

  visitFunction_body(ctx: Function_bodyContext): Function_Body {
    const statements: (Conditional | Expression | StaticFunction | undefined)[] = [];
    ...
    let returnValNum;
    const returnValExpression = <Expression> (returnValFunc.expression()?.accept(this));
    const returnValStaticFunc = <StaticFunction> (returnValFunc.static_function()?.accept(this));
    const returnValFuncCall = <Function_Call> (returnValFunc.function_call()?.accept(this));
    const returnValFormStateAccess = <FormStateAccess> (returnValFunc.form_state_access()?.accept(this));
    ...
    const returnValue = returnValVarName || returnValString || returnValNum || returnValExpression ||
        returnValStaticFunc || returnValFuncCall || returnValFormStateAccess || returnValArray;
    if (returnValue) {
  ...
  visitStatement(ctx: StatementContext): Conditional | Expression | StaticFunction | undefined {
    ...
    const statementStaticFunction = <StaticFunction> ctx.static_function()?.accept(this);
  ...
  visitFunction_call(ctx: Function_callContext): Function_Call {
    return new Function_Call(new VariableName(ctx.VARIABLE_NAME().text),
        ctx.function_params().accept(this));
  }

  visitConditional(ctx: ConditionalContext): Conditional {
    let ifCond = new If_Cond(<Expression> ctx.if_cond()?.condition().expression().accept(this),
        ctx.if_cond()?.cond_body().accept(this));
    let elseIfList: Else_If_Cond[] = [];
    for(const e of ctx.else_if_cond()) {
      elseIfList.push(new Else_If_Cond(e.condition().expression().accept(this),
  ...
  visitCond_body(ctx: Cond_bodyContext): Cond_Body {
    ...
    if (returnValIf) {
      ...
      const returnValExpression = <Expression>(returnValIf.expression()?.accept(this));
     if (returnValVarNameString) {
        returnValVarName = new VariableName(returnValVarNameString);
      } else if (returnValNumString) {
        returnValNum = Number(returnValNumString);
  ...
  visitHeader_field(
    ...
    const headerExpression = <Expression>(
  ...
  visitInstructions_field(
    ...
    const instructionExpression = <Expression>(
      ctx.expression()?.accept(this)
    );
  ...
  visitDisplayQuestions_field(ctx: DisplayQuestions_fieldContext): string | number | undefined {
  ...
  visitQuestion(ctx: QuestionContext): Question {
    ...
    for (let questionFields of ctx.question_fields().question_field()) {
      if (questionFields.id_field()) {
        let id = questionFields.id_field()?.STRING()?.text || questionFields.id_field()?.expression()?.accept(this);
      } else if (questionFields.type_field()) {
        let type = questionFields.type_field()?.text;
      } else if (questionFields.label_field()) {
        let label = <string | Expression | VariableName | Function_Call | undefined>(
            questionFields
                .label_field()
                ?.accept(this)
        );
        ...
        isRequired = (questionFields.isRequired_field()?.boolean().text == "true")
      } else if (questionFields.correctAnswer_field()) {
        correctAnswer = <string | number | Regex | Expression | Function_Call | undefined>(
            questionFields
                .correctAnswer_field()
                ?.accept(this)
        );
      } else if (questionFields.variables()?.variables_object()) {
        varsObject = <VariablesArray>questionFields
            .variables()
            ?.variables_object()
            .accept(this);
      }
  ...
  visitLabel_field(
    ...
    const labelExpression = <Expression>(
      ctx.expression()?.accept(this)
    );
  ...
  visitOptions_field(ctx: Options_fieldContext): VariableName | Function_Call | ArrayCustom | undefined {
  ...
  visitDisplayIf_field(
    ...
    const displayIfExpression = <Expression>(
      ctx.expression()?.accept(this)
    );
  ...
  visitCorrectAnswer_field(
    ...
    const correctAnswerExpression = <Expression>(
      ctx.expression()?.accept(this)
    );
  ...
  visitExpression(ctx: ExpressionContext): Expression {
    const expressionValue = Number(ctx.NUM()?.text) || ctx.STRING()?.text ||
        ctx.unscoped_expression()?.accept(this) || ctx.scoped_expression()?.accept(this);
    return new Expression(expressionValue);

  }

  visitScoped_expression(ctx: Scoped_expressionContext): ScopedExpression {
    let firstScopedExpression: ScopedExpression = ctx.first_scoped_expression()?.scoped_expression()?.accept(this)
    ...
    for (let s of ctx.extended_scoped_expression()) {
      let operator: string = s.scoped_operator()?.math_op()?.text || s.scoped_operator()?.num_equality_op()?.text || "";
      scopedExpressionDictionary[operator] = s.expression()?.accept(this);
    }
    return new ScopedExpression(firstScopedExpression, scopedExpressionDictionary);
  }

  //TODO: Assumes evaluator will handled undefined case
  visitUnscoped_expression(ctx: Unscoped_expressionContext): UnscopedExpression {
    ...
    if (mathExpressionStr) {
      return new UnscopedExpression(new MathExpression(mathExpressionStr), mathExpressionStr);
    } else if (stringExpressionStr) {
      return new UnscopedExpression(new StringExpression(stringExpressionStr), stringExpressionStr);
    } else {
      return new UnscopedExpression(new BooleanExpression(booleanExpressionStr), booleanExpressionStr);
    }
  }
  ...
  visitString_expression(ctx: String_expressionContext): string {
    ...
    if (finalExpression) {
      const strExpressionVal1 = finalExpression.string_expression_val1().STRING()?.text ||
          finalExpression.string_expression_val1().VARIABLE_NAME()?.text;
      const strValue =
        strExpressionVal1 +
        finalExpression.PLUS().text;
      if (finalExpression.string_expression_val2()) {
        const strExpressionVal2 = finalExpression.string_expression_val2()?.STRING()?.text ||
      ...
      const strVal1 = extendedExpression.string_expression_val1()?.STRING()?.text ||
          extendedExpression.string_expression_val1()?.VARIABLE_NAME()?.text;

      return (
        "" + strVal1 + extendedExpression.PLUS()?.text +
  ...
  visitMath_expression(ctx: Math_expressionContext): string {
    ...
    if (finalExpression) {
      const mathVal1 = finalExpression.math_expression_val1()?.NUM()?.text ||
          finalExpression.math_expression_val1()?.VARIABLE_NAME()?.text;
      const mathVal2 = finalExpression.math_expression_val2()?.NUM()?.text ||
          finalExpression.math_expression_val2()?.VARIABLE_NAME()?.text;
      return (
        "" + mathVal1 +
        finalExpression.math_op()?.text +
        mathVal2
      );
    } else if (extendedExpression) {
      const mathVal1 = extendedExpression.math_expression_val1()?.NUM()?.text ||
          extendedExpression.math_expression_val1()?.VARIABLE_NAME()?.text;
      return (
        "" + mathVal1 +
        extendedExpression.math_op()?.text +
        extendedExpression
          .math_expression()?.accept(this)
      );
    }
    return "";
  }
  ...
  visitBoolean_expression(ctx: Boolean_expressionContext): string | undefined {
    const stringBooleanExpression = ctx.string_boolean_expression()?.accept(this);
  ...
  visitString_boolean_expression(ctx: String_boolean_expressionContext): string {
    let strVal1 = ctx.string_expression_val1().STRING()?.text ||
        ctx.string_expression_val1().VARIABLE_NAME()?.text;
    let finalStr = strVal1 + ctx.string_equality_op().text;
    let strVal2 = (ctx.string_expression_val2()?.STRING()?.text ||
        ctx.string_expression_val2()?.VARIABLE_NAME()?.text) ||
        (ctx.string_boolean_expression_extended()?.string_equality_op().text +
    ctx.string_boolean_expression_extended()?.string_boolean_expression().accept(this));
    return finalStr + strVal2;
  }

  visitNum_boolean_expression(ctx: Num_boolean_expressionContext): string {
    let strVal1 = ctx.math_expression_val1().NUM()?.text ||
        ctx.math_expression_val1().VARIABLE_NAME()?.text;
    let finalStr = strVal1 + ctx.num_equality_op().text;
    let strVal2 = (ctx.math_expression_val2()?.NUM()?.text ||
            ctx.math_expression_val2()?.VARIABLE_NAME()?.text) ||
        (ctx.num_boolean_expression_extended()?.num_equality_op().text +
            ctx.num_boolean_expression_extended()?.num_boolean_expression().accept(this));
    return finalStr + strVal2;
  }

  visitGoTo_field(ctx: GoTo_fieldContext): string | Expression | Function_Call | undefined {
    const goToString = ctx.STRING()?.text;
    const goToExpression = <Expression>(
        ctx.expression()?.accept(this)
    );
